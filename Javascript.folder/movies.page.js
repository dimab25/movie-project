  ...
  fetch(url_GenreList)
    ...
    .then((data) => {
      ...
      const result = uniqueGenresArray.map((id) => {
        const match = genreList.find((obj) => obj.id === id);
        return match ? { id, name: match.name } : { id, name: null };
      });
...
const filterByDropDown = (movies) => {
  ...
  fetch(url_GenreList)
    ...
    .then((data) => {
      const genreList = data.genres;
      console.log("genrelist3", genreList);
      console.log(selectedGenre);
      const resultNew = genreList.find((obj) => obj.name === selectedGenre);
      // filter
      const filteredMovies = movies.filter((movie) => {
        return movie.genre_ids.includes(resultNew.id);
      });
